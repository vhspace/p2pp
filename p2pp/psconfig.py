...
def parse_config_parameters():
    ...
    for idx in range(len(v.input_gcode) - 1, -1, -1):
        ...
        if "generated by OrcaSlicer" in gcode_line:
            try:
                s1 = gcode_line.split("+")
                s2 = s1[0].split(" ")
                v.orcaslicer_version = s2[-1]
                gui.create_logitem("File was created with OrcaSlicer version:{}".format(v.orcaslicer_version))
            except (ValueError, IndexError):
                pass
            continue
